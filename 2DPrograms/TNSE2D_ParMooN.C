...
//       cout<<TDatabase::TimeDB->TIMESTEPLENGTH<<"\n"; 
  ...
  std::vector<int> bdid = {0,2};
  SystemMatrix->PickFreeSlipDOFs(bdid);
...
//======================================================================
     for(j=1;j<=Max_It;j++)
      {   
        ...
        SystemMatrix->modifyMatrixFreeSlip();
...
>>>>>>> Stashed changes
	...
	{ // time cycle
		...
		std::copy(sol,sol+N_U,solVector_U + (m-1)*N_U);
		std::copy(sol+N_U,sol+(2*N_U),solVector_V+ (m-1)*N_U);
	...
	FTLE *ftle = new FTLE(Velocity, 4,VelocityAll_U,VelocityAll_V);
