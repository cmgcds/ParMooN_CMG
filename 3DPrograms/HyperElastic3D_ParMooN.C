...
//#include <HyperElastic_MGLevel1.h>
double bound = 0;
double timeC = 0;
...
int main(int argc, char* argv[])
{
  ...
  int LEVELS, mg_level, mg_type, Max_It;
  double limit;
...
//     Database->WriteTimeDB();
  ...
  /* meshgenerator */
 // THIVIN  clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &start);
  ...
  // THIVIN clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &end);

// 	 // THIVIN double initial_mesh_time = time_taken(start,end);
...
//======================================================================   
     int N_DOF = FeSpaces[i]->GetN_DegreesOfFreedom();
     int N_Active = FeSpaces[i]->GetActiveBound();
