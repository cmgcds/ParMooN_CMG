...
int main(int argc, char* argv[])
{
  ...
  // ======================================================================
    ...
    TSystemTNSE2D *SystemMatrix_NSE;
    TAuxParam2D *aux, *NSEaux_error;

    int N_SubSteps_cd, ORDER, N_T, N_G;
    int N_Active; 
...
// fespaces for velocity and pressure  
        GetVelocityAndPressureSpace(coll,
                                    BoundCondition,
                                    mortarcoll,
                                    Velocity_FeSpace,
                                    Pressure_FeSpace,
...
//=========================================================================  
        ...
        N_T = Temperature_FeSpace->GetN_DegreesOfFreedom();
        N_Active =  Temperature_FeSpace->GetActiveBound();
    
        OutPut("DOF THETA    : "<< setw(10) << N_T  << endl);
    
        OutPut("Total Dof all: "<< setw(10) << (N_TotalDOF+N_T)  << endl); 
        OutPut("dof active   : "<< setw(10) << N_Active << endl);    

//========================================================================
//Construct Solution Space
//========================================================================
        sol = new double[N_TotalDOF+N_T]; 
        rhs = new double[N_TotalDOF+N_T];
        oldrhs = new double[N_TotalDOF+N_T]; 
//======================================================================
...
//======================================================================
        ...
        memset(sol_cd, 0, N_T*SizeOfDouble);
        memset(rhs_cd, 0, N_T*SizeOfDouble);

        Temperature = new TFEFunction2D(Temperature_FeSpace, TString, TString, sol_cd, N_T);
//======================================================================
...
//======================================================================
        ...
        // Disc type: GALERKIN 
        // Solver: AMG_SOLVE (or) GMG  (or) DIRECT  
        SystemMatrix_NSE = new TSystemTNSE2D(Velocity_FeSpace, Pressure_FeSpace, Velocity, Pressure, sol, rhs, Disctype, NSEType, DIRECT
#ifdef __PRIVATE__  
        ...
        // turbulent viscosity must be computed
            ...
            aux =  new TAuxParam2D(TimeNSN_FESpaces2,
                                TimeNSN_Fct2,
                                TimeNSN_ParamFct2,
                                TimeNSN_FEValues2,
                                fesp,
        ...
        {
            NSEaux_error =  new TAuxParam2D(TimeNSN_FESpaces2,
                                            TimeNSN_Fct2,
                                            TimeNSN_ParamFct2,
                                            TimeNSN_FEValues2,
                                            fesp,
...
//======================================================================
        ...
        SystemMatrix_NSE->Init(LinCoeffs, BoundCondition, U1BoundValue, U2BoundValue, aux, NSEaux_error);

        // assemble M, A matrices and rhs 
        SystemMatrix_NSE->Assemble(sol, rhs);
...
//             cout << TDatabase::ParamDB->DSType << endl;
//             exit(0);
        }
    ...
    {       
        ...
        for(l=0;l<N_SubSteps;l++) // sub steps of fractional step theta
        {
            ...
            {            
                ...
                if(m!=1)
                { SystemMatrix_NSE->AssembleRhs(sol, rhs); }
                else
                { SystemMatrix_NSE->Assemble(sol, rhs);  }
        
                //scale B matices and assemble NSE-rhs based on the \theta time stepping scheme 
                SystemMatrix_NSE->AssembleSystMat(tau/oldtau, oldrhs, rhs, sol); 
                oldtau = tau;
                ...
                SystemMatrix_NSE->GetTNSEResidual(sol, defect);
...
#endif
// ====================================================================== 
// Solve the system Nonlinear iteration of fixed point type
// ======================================================================
            for(j=1;j<=Max_It;j++)
            {
...
#endif
                {
                    ...
                    //assemble the system matrix with given aux, sol and rhs 
                    SystemMatrix_NSE->AssembleANonLinear(sol, rhs);    
...
//======================================================================    
        ...
        {   
            ...
            SystemMatrix_NSE->MeasureTNSEErrors(ExactU1, ExactU2, ExactP, AllErrors);
