...
#include "../TNSE_2D/2Phase_SurfDrop.h"
...
//   double Yi[6] = {y, ty, 0., 0., 6., 6.};
   ...
   double Xi[6] = {x, tx, 0.0, 0.5, 0.5, 0.0};
   double Yi[6] = {y, ty, 0.0, 0.0, 2.0, 2.0};
     ...
     for(l=0;l<k;l++)
      {
       ...
       else if(Joint->GetType() == IsoInterfaceJoint && Me->GetRegionID()==0)
        {
...
//   cout<<"N_U : "<<N_DOFs[0][0]<<"\tN_P : "<<N_DOFs[1][0]<<"\tN_G : "<<N_DOFs[2][0]<<"\n";
//  cout<<"Sol_u : "<<Ddot(2*N_DOFs[0][0], Sol[0][0], Sol[0][0])<<"\n";
//       cout<<"Sol_p : "<<Ddot(N_DOFs[1][0], Sol[0][0]+2*N_DOFs[0][0], Sol[0][0]+2*N_DOFs[0][0])<<"\n";
//       cout<<"Sol_w : "<<Ddot(2*N_DOFs[2][0], Sol[2][0], Sol[2][0])<<"\n";
// 
//       exit(0);
//   
...
//   double Yi[6] = {0.75, .25, 0., 0., 6., 6.};
     double Xi[6] = {0.0, 0.0, 0.0, 0.5, 0.5, 0.0};
   double Yi[6] = {0.75, 0.25, 0.0, 0.0, 2.0, 2.0};
  T_a = .25;
  T_b = .25;
  C_x = 0.0;  // center of the inner phase
 ...
 // remove all existing cells and joints
  ...
  for (i=0;i<N_G;i++)
     NewVertices[i] = new TVertex(Coordinates[2*i], Coordinates[2*i+1]);
      ...
      // set bounding box : Manually set it
  left = Xi[2];
  bottom = Yi[2];
  right = Xi[4];
  top = Yi[4];
     ...
     for(l=0;l<k;l++)
      {
       ...
       else if(Joint->GetType() == IsoInterfaceJoint && Me->GetRegionID()==0) // w.r.t inner phase
        {
         IntFaceJoint = (TIsoInterfaceJoint *)Joint;
         BoundComp = IntFaceJoint->GetBoundComp();
         comp=BoundComp->GetID();
...
//          cout<<"Before calculation of grid velocity!!!!!!!\n";
//       cout<<"Sol[2][0] : "<<Ddot(2*N_DOFs[2][0], Sol[2][0], Sol[2][0])<<"\n";
// 	 double *sol_mesh_P1 = new double[N_DOFs[2][1]];
//       sol_mesh_P1 = VeloVect[1][1]->GetValues();
//       cout<<"FeVecFunctions[2][1] : "<<Ddot(2*N_DOFs[2][1], sol_mesh_P1, sol_mesh_P1)<<"\n";
//       
//       cout<<"gridsol[1] : "<<Ddot(2*N_DOFs[2][1], gridsol[1], gridsol[1])<<"\n";
//       cout<<"gridrhs[1] : "<<Ddot(2*N_DOFs[2][1], gridrhs[1], gridrhs[1])<<"\n";
//       cout<<"N_G : "<<N_DOFs[2][0]<<"\tN_G_P1 : "<<N_DOFs[2][1]<<"\tN_G_P2 : "<<N_DOFs[2][2]<<"\tN_U : "<<N_DOFs[0][0]<<"\n";
//        
//       double *sol_vel_check = new double[N_DOFs[0][0]];
//       sol_vel_check = VeloVect[0][0]->GetValues(); 
//       cout<<"FeVecFunctions[0][0] : "<<Ddot(2*N_DOFs[0][0], sol_vel_check, sol_vel_check)<<"\n";
//       
//      cout<<"Ddot pos_P1 : "<<Ddot(2*N_DOFs[2][1], pos_P1, pos_P1)<<"\n"; 
//       cout<<"Ddot auxpos_P1 : "<<Ddot(2*N_DOFs[2][1], auxpos_P1, auxpos_P1)<<"\n";
...
//       cout<<"After calculation of domain velocity\n";
//       cout<<"length : "<<length<<"\n";
//       cout<<"N_Cells_P1 : "<<N_Cells_P1<<"\n";
//       cout<<"N_Cells_P2 : "<<N_Cells_P2<<"\n";
      for(i=0;i<N_Cells_P2;i++)
       {
...
// // exit(0);
    ...
    //======================================================================
      ...
      cout<<"Sol_u : "<<Ddot(2*N_U, Sol[0][0], Sol[0][0])<<"\n";
      cout<<"Sol_p : "<<Ddot(N_P, Sol[0][0] + 2*N_U, Sol[0][0] + 2*N_U)<<"\n";      
...
//      	OutPut( "Remeshing required !!!!!!!!!! "<< endl);
// 	exit(0);
//    if((l==0) && ((m % 1) == 0))
//     {
...
// cout<<"After remeshing but before non-linear solve\n";
// cout<<"N_U : "<<N_DOFs[0][0]<<"\tN_P : "<<N_DOFs[1][0]<<"\tN_G : "<<N_DOFs[2][0]<<"\n";
//  cout<<"Sol_u : "<<Ddot(2*N_DOFs[0][0], Sol[0][0], Sol[0][0])<<"\n";
// cout<<"Sol_p : "<<Ddot(N_DOFs[1][0], Sol[0][0]+2*N_DOFs[0][0], Sol[0][0]+2*N_DOFs[0][0])<<"\n";
//       cout<<"Sol_w : "<<Ddot(2*N_DOFs[2][0], Sol[2][0], Sol[2][0])<<"\n";
...
//     }
 ...
 OutPut( "Final state of the two phase system : "<< endl);
       if(FluidFlow)
       {
        Get_KE(VeloVect[0][0], Params);
        CurrVolume = 2.*Pi*Params[0];
