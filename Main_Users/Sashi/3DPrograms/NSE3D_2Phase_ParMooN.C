...
int main(int argc, char* argv[])
{
  ...
  // ======================================================================
  int i, j, N_Cells, N_Cells_P0, N_Cells_P1,  N_U, N_P, N_TotalDOF, img=1, pressure_space_code;
  int Max_It, NSEType, velocity_space_code, Phase_ID;
  ...
  TBaseCell *Cell, **Cells_P0, **Cells_P1;
  int profiling;
#ifdef _MPI
  ...
  TCollection *coll, *mortarcoll = NULL, *Coll_P0,  *Coll_P1;
...
//     Database->WriteTimeDB();
  ...
  if(TDatabase::ParamDB->MESH_TYPE==0)
    {Domain->ReadGeo(TDatabase::ParamDB->GEOFILE); } // ParMooN  build-in Geo mesh
...
//     velocity_space  =  new TFESpace3D(Coll_P0, Name, UString, BoundCondition, 1);  
...
//=========================================================================  
     if(mg_type==1) // lower order FE on coarse grids 
      {
       Velocity_FeSpace[i] = new TFESpace3D(Coll_P1, Name, UString, BoundCondition, Non_USpace,1);
       Pressure_FeSpace[i] = new TFESpace3D(Coll_P1, Name, PString, BoundCondition, DiscP_PSpace,0);    
       
       if(i==LEVELS-1) // higher order on fine level
        {
         GetVelocityAndPressureSpace3D(Coll_P1, BoundCondition, velocity_space,
