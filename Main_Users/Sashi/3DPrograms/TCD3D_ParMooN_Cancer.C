...
for(ii=0;ii<Levels; ii++)
  ...
  //U solution spaces
    ...
    for(i=0;i<N_Cells;i++)
    {
      ...
      N_U_LocalDof = N_BaseFunct[U_FEId];
...
// 	  OutPut("update rhs done " << Ddot(N_V_LocalDof,LocRhs,LocRhs) << endl);
	  
	  ufact = alphaUbeta*test000;
	  //cout<<"Alpha * Uvalue is "<<alpha*U_value + beta<<endl;
...
//      OutPut("update rhs done " << Ddot(TestDOF,VRHS,VRHS) << endl);
...
// 	    cout << TestDOF <<" A: "<< LocMatrixA[l*N_V_LocalDof+m] <<endl;
	   break;
	 } //if(KCol[n] == DOF[m])
