...
//
// derived from a Stokes problem given in
//       D.Braess and R.Sarazin, An efficient smoother for the Stokes
//          problem, Appl. Num. Math., 23: 3-19, 1997
//

void ExampleFile()
{
  OutPut("Example: BraessSarazinNSE.h" << endl) ;
}
               ...
               double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    if(TDatabase::ParamDB->STOKES_PROBLEM)
    {
      coeff[0] = 1;
      coeff[1] = 0;               // f1
      coeff[2] = 4*cos(x)*cos(y); // f2
    }
    else
    {
      coeff[0] = 1;
      coeff[1] = sin(x)*cos(x);               // f1
      coeff[2] = -sin(y)*cos(y)+4*cos(x)*cos(y); // f2
    }
  }
}
