...
void ExampleFile()
{
  OutPut("Example: Poly_u_Sin_p_scaled.h,"); 
...
void ExactP(double x, double y, double *values)
{
  ...
  values[0] = cos(2*Pi*x)*sin(Pi*y);
  values[1] = -2*Pi*sin(2*Pi*x)*sin(Pi*y);
  values[2] = Pi*cos(2*Pi*x)*cos(Pi*y);
  values[3] = 0;
  values[0] *= scale;
  values[1] *= scale;
  values[2] *= scale;
               ...
               double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[1] = -nu*(4.0*t3*y*t6+16.0*t8*t9+4.0*t11*y*t6+4.0*t3*t14*t5
               ...
               -2*Pi*sin(2*Pi*x)*sin(Pi*y)*scale;
