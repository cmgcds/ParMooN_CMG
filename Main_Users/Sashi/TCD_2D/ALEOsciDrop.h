...
void ExampleFile()
{
  OutPut("Example: ALEOsciDrop.h" << endl) ;
...
void  ModifyBdCoords(int *N_MovVert, TVertex **MovBoundVert, TIsoBoundEdge **Free_Joint,  double * Iso_refX, double t)
{
 ...
 double x, y, Xi[2], Yi[2], C_x, C_y;//, t=TDatabase::TimeDB->CURRENTTIME;
 double disp, a, b, theta, h;
  ...
  UpdateBound  = (TBdLine*)BoundPart->GetBdComp(0);
  UpdateIntface = (TBdCircle*)BoundPart->GetBdComp(1); 
   ...
   for(i=0;i<=N_MovVert[0];i++)
    {
     ...
     //cout<<i << "  " << (180/Pi)*theta <<  " x " << x << " y : "<< y<<   " y_new : "<< y + disp<<endl;
     if(fabs(x)<1e-5) x=0.;
     if(fabs(y)<1e-5) y=0.;
 ...
 // update iso points
  ...
  MovBoundVert[N_MovVert[0]]->GetCoords(Xi[0], Yi[0]); 
  MovBoundVert[0]->GetCoords(Xi[1], Yi[1]); 
   
  // Solid Bound startx, starty, x length and y length
  UpdateBound->SetParams(Xi[0], Yi[0], Xi[1]-Xi[0],Yi[1]-Yi[0]);

  C_x=0.;
  C_y=1.;
  // Free boundary xmid, ymid, radius_a, radius_b, start angle, end angle
  UpdateIntface->SetParams(C_x, C_y, a, b, -Pi/2., Pi/2.);    
  
   N = N_MovVert[1];
   h = (Yi[1] - Yi[0])/(double)N;
//    cout << " h " << h <<endl;
   for(i=1;i<N_MovVert[1];i++)
    {  
      
     MovBoundVert[N_MovVert[0]+i]->GetCoords(x, y);       
     y = Yi[0] + h*(double)i;
     MovBoundVert[N_MovVert[0]+i]->SetCoords(0., y); 
      
     //cout<<i<<  "x " << x << " y : "<< y << "Ynew " << Yi[0] + h*(double)i<<endl; 
    }
    
 
  for(k=0;k<N_MovVert[1];k++)
   if(k==N_MovVert[1]-1)
    { Free_Joint[k]->UpdateParameters(MovBoundVert[k], MovBoundVert[0]); }
   else
    { Free_Joint[k]->UpdateParameters(MovBoundVert[N_MovVert[0]+k], MovBoundVert[N_MovVert[0]+k+1]); }
   
//    cout<<  N_MovVert[0] <<  " ReMaxy : "<<endl;
//    exit(0);
} // ModifyBdCoords

// sorting curved surfrace vertices - general case
void Sort_Imping(TBaseCell **cell, TVertex **Vertex, int *CellNo, int *EdgeNo, int *N_vert,
                     double X0, double Y0 )
{
 int i, j, k, temp, test=0, N;
...
//        cout << " sorting " << x << ' ' << y<<endl;
  ...
  for(i=0;i<N_vert[0]-1;i++)
   {
    test = 0; 
    k = cell[i]->GetN_Edges();
    cell[i]->GetVertex((EdgeNo[i]+1) % k)->GetCoords(x, y);

     for(j=i+1;j<N_vert[0];j++)
      {
 ...
 // finding the right(starting) vertex
   for(i=N_vert[0];i<N;i++)
   {
    Vertex[i]->GetCoords(x, y);

    if(sqrt((x-X0)*(x-X0) +(y-Y0)* (y-Y0))<1e-5  )
      {
       //cout <<EdgeNo[N_vert[0]] << " sorting " << x << ' ' << y<<endl;
       temp_vert = Vertex[N_vert[0]];
       Vertex[N_vert[0]] = Vertex[i];
       Vertex[i] = temp_vert;

       temp_cell = cell[N_vert[0]];
       cell[N_vert[0]] = cell[i];
       cell[i] = temp_cell;
       
       temp = EdgeNo[N_vert[0]];
       EdgeNo[N_vert[0]] = EdgeNo[i];
       EdgeNo[i] = temp;
       
       temp = CellNo[N_vert[0]];
       CellNo[N_vert[0]] = CellNo[i];
       CellNo[i] = temp;        
...
{
 int i, j, k, l, m0, m1, m2, N_Cells, comp, N_MovVert_all;
 int ORDER, VSP, temp, BdId;
...
//   N_MovVert = new int[2];
  ...
  N_MovVert[1] = 0;
    ...
    for(j=0;j<N_Cells;j++)
	  ...
	  { 
           BdId  = (((TBoundEdge *)Joint)->GetBoundComp())->GetID();
	   if(BdId==0)
	   { N_MovVert[1]++; }
	   else if(BdId==1)
	   { N_MovVert[0]++; }  
        ...
        N_MovVert_all = N_MovVert[0] + N_MovVert[1];
      cout<<   " N_MovVert_all " << N_MovVert_all << endl; 
// exit(0);
     // free bound
     Free_Joint = new TIsoBoundEdge*[N_MovVert_all];
     MovBoundVert = new TVertex*[N_MovVert_all];
     Free_Cells = new TBaseCell*[N_MovVert_all];
     IsoCellEdgeNos[0]  = new int [N_MovVert_all];
     IsoCellEdgeNos[1]  = new int [N_MovVert_all];
     ...
     for(j=0;j<N_Cells;j++)
      {
	    ...
	    if(BdId==0)
	     {
              Free_Joint[m1] = (TIsoBoundEdge *)Joint;
              MovBoundVert[m1] = Me->GetVertex(l);
              Free_Cells[m1] = Me;
              IsoCellEdgeNos[0][m1] = j;
              IsoCellEdgeNos[1][m1] = l;
	    
	      MovBoundVert[m1]->GetCoords(x, y);
              //cout<<"cell " << j<<  " x : "<< x << " y : "<< y<< endl;
  ...
  // sort         
   Sort_Imping(Free_Cells, MovBoundVert, IsoCellEdgeNos[0], IsoCellEdgeNos[1], N_MovVert, 0, 0);
...
void  TriaReMeshGen(TDomain *&Domain)
{
  ...
  N_Interface_Vert = 300;    //Freesurf except end point
  N_Verti = 50;       // number of vertical BD vertices
  
  N_BDVertices = N_Verti+N_Interface_Vert;
...
//     cout<<" x : "<< In.pointlist[2*In_Index] << " y : "<< In.pointlist[2*In_Index+1] <<endl;
  ...
  C_y = 1.0, // center of the inner phase
  phi1 = -Pi/2.; // end degree value of interface
  phi2 =  Pi/2.;
 
  s = (phi2- phi1)/(double)N_Interface_Vert; 
  
  // points and segments on the interface (marker=1)
