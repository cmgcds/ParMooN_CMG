...
//==========================================================================
// academic test case for verification of bulk example
// velocy field v=(0.5-y,x-0.5)sin(Pi t), counterclockwise rotation about the center
...
void ExampleFile()
{
  OutPut("Example: Bulk_Academic_Test.h" << endl);
}
...
void Exact(double x, double y, double *values)
{
  ...
  values[0] = 0.6*(sin(2*Pi*x)*sin(2*Pi*y)+1)*sin(Pi*t/2.0)*t;
  values[1] = 0.6*(2*Pi*cos(2*Pi*x)*sin(2*Pi*y))*sin(Pi*t/2.0)*t;
  values[2] = 0.6*(2*Pi*sin(2*Pi*x)*cos(2*Pi*y))*sin(Pi*t/2.0)*t;
  values[3] = 0;
}
...
double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[0] = eps;
    coeff[1] = (0.5-y) * sin(Pi*t);
    coeff[2] = (x-0.5) * sin(Pi*t);
    coeff[3] = 0;

    // rhs from academic
    rhs = 0.6*(sin(2*Pi*x)*sin(2*Pi*y)+1)*(cos(Pi*t/2) * Pi/2*t+sin(Pi*t/2));
    rhs += ((0.5-y) * sin(Pi*t)) * 0.6*(2*Pi*cos(2*Pi*x)*sin(2*Pi*y))*sin(Pi*t/2)*t;
    rhs += ((x-0.5) * sin(Pi*t)) * 0.6*(2*Pi*sin(2*Pi*x)*cos(2*Pi*y))*sin(Pi*t/2)*t;
    rhs += - eps * 0.6*(-4*Pi*Pi*sin(2*Pi*x)*sin(2*Pi*y))*sin(Pi*t/2)*t;
    rhs += - eps *  0.6*(-4*Pi*Pi*sin(2*Pi*x)*sin(2*Pi*y))*sin(Pi*t/2)*t;
    // rhs
    coeff[4] = rhs;
