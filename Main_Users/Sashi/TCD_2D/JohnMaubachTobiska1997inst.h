...
void ExampleFile()
{
  OutPut("Example: JohnMaubachTobiska1997inst.h" << endl) ;
...
void Exact(double x, double y, double *values)
{
 ...
 double gxy_l,v_x,v_y,c_m,para;
  ...
  values[0] = sin(Pi*t)* g1xy*g2xy;
  values[1] = sin(Pi*t)*(g1xy_x*g2xy + g1xy*g2xy_x);
  values[2] = sin(Pi*t)* (g1xy_y*g2xy + g1xy*g2xy_y);
  gxy_xx  =  g1xy_xx*g2xy + 2*g1xy_x*g2xy_x + g1xy*g2xy_xx;
  gxy_yy  =  g1xy_yy*g2xy + 2*g1xy_y*g2xy_y + g1xy*g2xy_yy;
  values[3]  = sin(Pi*t)*(gxy_xx + gxy_yy);
 }
        ...
        double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    gxy     =  sin(Pi*t)*g1xy*g2xy;
    gxy_x   =  sin(Pi*t)*(g1xy_x*g2xy + g1xy*g2xy_x);
    gxy_xx  =  sin(Pi*t)*(g1xy_xx*g2xy + 2*g1xy_x*g2xy_x + g1xy*g2xy_xx);
    gxy_y   =  sin(Pi*t)*(g1xy_y*g2xy + g1xy*g2xy_y);
    gxy_yy  =  sin(Pi*t)*(g1xy_yy*g2xy + 2*g1xy_y*g2xy_y + g1xy*g2xy_yy);
    gxy_l  =  gxy_xx + gxy_yy;

    //OutPut(gxy << " " << gxy_x << " "<< gxy_y << " " << gxy_l<< " ");
  
    coeff[0] = eps;
    coeff[1] = 2;
    coeff[2] = 3;
    coeff[3] = 1;
    coeff[4] = Pi*cos(Pi*t)*g1xy*g2xy-coeff[0]*gxy_l+coeff[1]*gxy_x+coeff[2]*gxy_y+coeff[3]*gxy; 

    //OutPut(coeff[4] << endl);

    // for schemes needing the residual
    gxy     =  sin(Pi*tau)*g1xy*g2xy;
    gxy_x   =  sin(Pi*tau)*(g1xy_x*g2xy + g1xy*g2xy_x);
    gxy_xx  =  sin(Pi*tau)*(g1xy_xx*g2xy + 2*g1xy_x*g2xy_x + g1xy*g2xy_xx);
    gxy_y   =  sin(Pi*tau)*(g1xy_y*g2xy + g1xy*g2xy_y);
    gxy_yy  =  sin(Pi*tau)*(g1xy_yy*g2xy + 2*g1xy_y*g2xy_y + g1xy*g2xy_yy);
    gxy_l  =  gxy_xx + gxy_yy;
    coeff[5] = Pi*cos(Pi*tau)*g1xy*g2xy-coeff[0]*gxy_l+coeff[1]*gxy_x+coeff[2]*gxy_y+coeff[3]*gxy; 
  }
}
