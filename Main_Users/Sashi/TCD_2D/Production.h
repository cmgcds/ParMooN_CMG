...
#define __TCD_2D_PRODUCTION__

void ExampleFile()
{
  OutPut("Example: Production.h with reactive coefficient " << 
	 TDatabase::ParamDB->P7 << endl);
}
...
void InitialCondition(double x,  double y, double *values)
{
  double eps = 1e-6;
  values[0] = 1.01;
  if ((fabs(x)<1e-6))//&&(y>=0.4-eps)&&(y<=0.6+eps))
      values[0] = 1;      
}
...
double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[2] = 0;
    // reaction
    coeff[3] = TDatabase::ParamDB->P7;
    // rhs
    coeff[4] = TDatabase::ParamDB->P7;
    // rhs from previous time step
    coeff[5] = TDatabase::ParamDB->P7;
  }
}
...
double* &neum_to_diri_param)
{
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    for (j=0;j<N_V;j++)
    {
      ...
      if ((fabs(x[j])<eps)||(fabs(y[j])<eps)||(fabs(1-y[j])<eps))
      {
