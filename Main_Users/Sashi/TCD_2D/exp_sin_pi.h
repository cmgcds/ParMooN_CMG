...
void ExampleFile()
{
  OutPut("Example: exp_sin_pi.h" << endl); 
}
...
void Exact(double x, double y, double *values)
{
  ...
  values[0] = exp(sin(2*Pi*t))*(sin(2*Pi*x)*sin(2*Pi*y));
  values[1] = exp(sin(2*Pi*t))*2*Pi*cos(2*Pi*x)*sin(2*Pi*y);
  values[2] = exp(sin(2*Pi*t))*2*Pi*sin(2*Pi*x)*cos(2*Pi*y);
  //values[0] = 0;
  //values[1] = 0;
  //values[2] = 0;
  values[3] = 0;
        ...
        double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[4] = exp(sin(2*Pi*t))*2*Pi*cos(2*Pi*t)*(sin(2*Pi*x)*sin(2*Pi*y))
	- eps * exp(sin(2*Pi*t))*4*Pi*Pi*(-sin(2*Pi*x)*sin(2*Pi*y)-sin(2*Pi*x)*sin(2*Pi*y))
       + a * exp(sin(2*Pi*t))*2*Pi*cos(2*Pi*x)*sin(2*Pi*y)
       + b * exp(sin(2*Pi*t))*2*Pi*sin(2*Pi*x)*cos(2*Pi*y)
	+ c *  exp(sin(2*Pi*t))*(sin(2*Pi*x)*sin(2*Pi*y));
  }
}
