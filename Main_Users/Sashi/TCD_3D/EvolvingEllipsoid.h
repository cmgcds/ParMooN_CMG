...
//  double t=TDatabase::TimeDB->CURRENTTIME;
 ...
 double alph=exp(-6.*t);
 double a=1.*d0+amp*d0*sin(t);
 double b=1.*d0;
 double c=1.*d0;
 double da=amp*d0*cos(t);

//  double dd0=1.*d0;
//  double dalph=-6.*alph;
//  double db=0.*d0;
//  double dc=0.*d0;
//       D=sqrt( ((x1*x1)/(a*a)) + ((x2*x2)/(b*b)) + ((x3*x3)/(c*c)) );
//  double xn1=d0*x1/(a*D);
//  double xn2=d0*x2/(b*D);
//  double xn3=d0*x3/(c*D);

// rhs =  x1*x2*(dalph+alph*(((da/2.)/a)+((db/2.)/b))+alph*(
//                ((da/2.)/a)*(1.*d0-(xn1*xn1))+((db/2.)/b)*(1.-(xn2*xn2))
//               +((dc/2.)/c)*(1.-(xn3*xn3))) )
//        + dd0*alph*x1*x2*(    (2./(a*b*D*D)) + (1./D)*((1./a) + (1./b))
//        *(  (((1./a) + (1./b) + (1./c) )/D)- (1./(D*D*D))*
//         ( (x1*x1/(a*a*a)) + ((x2*x2)/(b*b*b)) + ((x3*x3)/(c*c*c)) )  )    );
// 
// cout << " rhs0  " << rhs << endl;

 D = ((x1*x1)/(a*a)) + ((x2*x2)/(b*b)) + ((x3*x3)/(c*c));
// // surface moving in Eulerian manner, with convection term
//  rhs = x1*x2*alph*( (da/(2.*a)) -6. + (da/(2.*a))*(1. - ( (x1*x1/(a*a))/( x1*x1/(a*a) + x2*x2 + x3*x3 ) ) )
//              + 2./(a*D) + (1./D)*(1./a + 1./b)*((1./a) + 2. - (1./D)*((x1*x1/(a*a*a)) + x2*x2 + x3*x3) ) );
// cout << " rhs1  " << rhs << endl;
// 
// // surface moving in Lagrangian manner, thus no convection term
//  rhs = x1*x2*alph*( -6. + (da/(2.*a))*(1. - ( (x1*x1/(a*a))/( x1*x1/(a*a) + x2*x2 + x3*x3 ) ) )
//              + 2./(a*D) + (1./D)*(1./a + 1./b)*((1./a) + 2. - (1./D)*((x1*x1/(a*a*a)) + x2*x2 + x3*x3) ) );
// cout << " rhs2  " << rhs << endl;


rhs = x1*x2*alph*( -6.);
// rhs += x1*x2*alph*( -6.)*(da/(2.*a)); // convection term 
rhs += x1*x2*alph*(da/(2.*a))*(1. - ( (x1*x1/(a*a))/( x1*x1/(a*a) + x2*x2 + x3*x3 ) ) );
rhs += x1*x2*alph*((2./(a*D)) + (1./D)*(1./a + 1./b)*((1./a) + 2. - (1./D)*((x1*x1/(a*a*a)) + x2*x2 + x3*x3) ));

// w\cdot\tangnabla\Gamma
// rhs -= (da/(2.*a)*x1*alph*(x2 - (x1*x1*x2/(a*D))*(1. + (1./a)) ));
// cout << " rhs3  " << rhs << endl;
...
void GetScale(double t, double old_t, double &value)
{
 double amp=TDatabase::ParamDB->P15;

 value =  (1. + amp*sin(t))/(1. + amp*sin(old_t)) ;
 value = sqrt(value);
}
          ...
          if(m>=N_Inner)
           {
            ...
            ValuesNewY[m] = RefPosY[m];
            ValuesNewZ[m] = RefPosZ[m];
// only at t=0
...
// evolving ellipsoid
//             ValuesNewY[m] =  PreviousPosY[m];
//             ValuesNewZ[m] =  PreviousPosZ[m];
// // expanding sphere
