...
#define __ROBINBC__ 
  ...
  OutPut("Example: TeraHertzBrain.h" << endl);
}
...
void BoundCondition(double x, double y, double z, BoundCond &cond)
{
   ...
   else if(TDatabase::ParamDB->P13==2)
   {
    if(y<-203. )  
     { cond = DIRICHLET; }
    else
        ...
        double **parameters, double **coeffs)
{
  ...
  // cout << "Region_ID = " << Region_ID << endl;
   ...
   if(Region_ID==1) //1-scalp and skull 
    ...
    {
      cout<< " No region info, see example file " << endl;
      exit(0);  
    } 
...
//    cout<< eps << " No " << " region " << char_L  << " info, see example file " << rhsfact  << endl;    
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[4] = 0;
     ...
     if(TDatabase::ParamDB->P13==0)
      { //Lside  
       Sourceradius = sqrt( (y-yp)*(y-yp) + (z-zp)*(z-zp));
       SourceCoord = -x;
      }
     else if(TDatabase::ParamDB->P13==1)
      { //Back side  
...
//       if(TDatabase::ParamDB->P14<coeff[5]) TDatabase::ParamDB->P14=coeff[5];
      // measure diameter of all cell which are within the Sourceradius
...
void ReadAdditionalNodes(char *NODE, tetgenio &InAddPts)
{
...
void ReadMeditMesh_Adapt(char *SMESH, tetgenio &In)
{  
  int i, j, k, m, dimension, N_FVert, N_Faces, N_Vertices;
  int BDComp_Min=10000000, N_AddVert, count, VertList[4], Fcount;
 ...
 std::ifstream dat(SMESH);
  ...
  N_AddVert = 6;
  In.numberofpoints = N_Vertices +  N_AddVert;
...
//      In.pointmtrlist[i] = 10.;
  ...
  // N_AddVert vertices will be added later
  ...
  while (!dat.eof())
  {
    ...
    for(i=0;i<N_Faces; i++)
     ...
     if( (i==1479 || i==1517  || i==1518 ||  i==1695 || i==1702 || i==1740  ) && N_AddVert>0) //       
     {
       X_=0.;
       Y_=0.;
       Z_=0.;       
             
       //first generate new vertex in the face
      for(j=0;j<N_FVert;j++)
       {
        dat >> k;
        m = k-1; // c numbering 
        VertList[j] = m; // store the orig order for later use
        X_ += In.pointlist[3*m];
        Y_ += In.pointlist[3*m + 1];
        Z_ += In.pointlist[3*m + 2];
 
//         cout<< m << " vert X: " <<In.pointlist[3*m] << " vert Y: " <<In.pointlist[3*m + 1] <<  " vert Z: " <<In.pointlist[3*m + 2]<<endl;  	
       }   
       dat >>  In.facetmarkerlist[i];  
      ...
      // add the additional vertex in thethe polygon
      m = N_Vertices + count;
      In.pointlist[3*m] = X_;
      In.pointlist[3*m+1] = Y_;
      In.pointlist[3*m+2] = Z_;       
      count++;
//       cout<< m << " vert X: " <<X_ << " vert Y: " <<Y_ <<  " vert Z: " <<Z_ <<endl;  
      
     //now input the ith face info
     P->numberofvertices = N_FVert;
     P->vertexlist = new int[P->numberofvertices]; 
     
     if(i==1479 || i==1517)
      {
       P->vertexlist[0] = VertList[0]; 
       P->vertexlist[1] =  m;   
       P->vertexlist[2] = VertList[2]; 
      }
     else if( i==1740  )
      {
       P->vertexlist[0] = VertList[2]; 
       P->vertexlist[1] =  m;   
       P->vertexlist[2] = VertList[1]; 
      }
     else
     {
       P->vertexlist[0] = VertList[0]; 
       P->vertexlist[1] =  VertList[1];   
       P->vertexlist[2] = m; 	       
     }
     
       cout<< m << " vert 1: " << P->vertexlist[0] << " vert 2: " <<P->vertexlist[1] <<  " vert 3: " <<P->vertexlist[2]<<endl; 
                  cout<<endl;
		  
/*    // add additional two faces
      F = &In.facetlist[N_Faces+Fcount];
      tetgenio::init(F);      
      F->numberofpolygons = 1;
      F->polygonlist = new tetgenio::polygon[F->numberofpolygons];
      F->numberofholes = 0;
      ...
      P->numberofvertices = N_FVert;
      P->vertexlist = new int[P->numberofvertices]; 
      P->vertexlist[0] = VertList[1]; 
      P->vertexlist[1] = VertList[2];   
      P->vertexlist[2] = m; 
      In.facetmarkerlist[N_Faces+Fcount] = In.facetmarkerlist[i];
      Fcount++;
   ...
   // now add other 2 faces
    ...
    // add additional two faces
     ...
     P = &F->polygonlist[0];
     tetgenio::init(P);       
     P->numberofvertices = 8;
     P->vertexlist = new int[P->numberofvertices]; 
     P->vertexlist[0] = 729;       
     P->vertexlist[1] = N_Vertices; 
     P->vertexlist[2] = 728;        
     P->vertexlist[3] = N_Vertices + 1; 
     P->vertexlist[4] = 409;           
     P->vertexlist[5] = N_Vertices + 2;  
     P->vertexlist[6] = 727;   
     P->vertexlist[7] = N_Vertices + 4;     
     
     In.facetmarkerlist[N_Faces+Fcount] = In.facetmarkerlist[i-1];
     Fcount++;   
     ...
     P->vertexlist = new int[P->numberofvertices]; 
     P->vertexlist[0] = 727;         
     P->vertexlist[1] = N_Vertices + 4; 
     P->vertexlist[2] = 410;              
     P->vertexlist[3] = N_Vertices + 3;  
     P->vertexlist[4] = 724;               
     P->vertexlist[5] = N_Vertices + 5; 
     P->vertexlist[6] = 723;            
     P->vertexlist[7] = N_Vertices + 2;     


     
       cout<< m << " vert: " << P->vertexlist[0] << " vert: " <<P->vertexlist[1] <<  " vert: " <<P->vertexlist[2]<< " vert: " << P->vertexlist[3] <<  " vert: " << P->vertexlist[4]<<endl; 
                  cout<<endl;
     ...
     P->vertexlist = new int[P->numberofvertices]; 
     P->vertexlist[0] = N_Vertices + 3;   
     P->vertexlist[1] = N_Vertices + 5;  
     P->vertexlist[2] = N_Vertices + 2;   
     P->vertexlist[3] = 727;    
//      P->vertexlist[5] =  N_Vertices + 4;   
   ...
   for(i=0;i<N_Vertices; i++)
    {
     ...
     if(TDatabase::ParamDB->P13==0)
      { //Lside
        //center point is 3.606596999999999830777142	93.27791100000000312775228	78.19347199999999986630428
       DispX = 3.606596999999999830777142;
       DispY = 0.;
       DispZ = 0.;
      }
     else if(TDatabase::ParamDB->P13==1)
      {
       //Backside
       //center point is 60388: 83.11056343315070193966676 12.29866318748098663604651	97.409675520607805765394
       DispX = 0.;
       DispY = 12.29866318748098663604651;
       DispZ = 0.;       
      }
     else if(TDatabase::ParamDB->P13==2)// default
      {
       //Front side 
       //center point is 986: 91.21127599999999802093953 214.3263880000000028758222 85.10373400000000287946023 
       DispX = 0.;
       DispY = 214.3263880000000028758222;
       DispZ = 0.;       
      }  
     else // default
      {
       //right side 
       //center point is 727: 177.8944370000000105846993	95.85110799999999642295734	81.84575200000000450017978
       DispX = 177.8944370000000105846993;
       DispY = 0.;
       DispZ = 0.;         
      }
     ...
     NewVertices[i] = new TVertex( (X - DispX), (Y - DispY), (Z- DispZ));

//      if(i==1062 || i==916 || i==341 || i==914 || i==162 || i==1063 || i==1061) // i==1062  as center LSide
//      if(i==60388 || i==65824 || i==60386 || i==230 || i==60394 || i==60391 || i==590) // i==60388  as center BackSide 
//       cout<< i << " vert X: " <<X << " vert Y: " <<Y <<" vert Z: " <<Z <<endl; 
     
//      if(i==986 || i==524 || i==993 || i==985 || i==546 || i==987 || i==988) // i==71212  as center Front 
//       cout<< i << " vert X: " <<X << " vert Y: " <<Y <<" vert Z: " <<Z <<endl; 
     
//      if(i==727 || i==410 || i==729 || i==728 || i==409 || i==723 || i==724) // i==727  as right  side 
//       cout<< i << " vert X: " <<X << " vert Y: " <<Y <<" vert Z: " <<Z <<endl; 
...
//     opts<<'r'; // -r  Reconstructs a previously generated mesh     
    opts<<"q"<<1.75; // quality mesh generation(q) with a specified quality bound
...
//   ReadMeditMesh(SMESH, In);
  ReadMeditMesh_Adapt(SMESH, In);  
// // // //     ReadMeditMesh_Adapt2(SMESH, In); 
...
//     std::ostringstream opts_New;
//     opts_New << " "; 
//   
//     opts_New.seekp(std::ios::beg);
// //     opts_New<<'p'; // Tetrahedralize the PLC. Switches are chosen to read a PLC (p)
//     opts_New<<'r'; // -r  Reconstructs a previously generated mesh     
// //     opts_New<<"q"<<1.49; // quality mesh generation(q) with a specified quality bound
// //     opts_New<<"a"<<0.1; // maximum volume constraint
// //     opts_New<<'i'; // Inserts a list of additional points into mesh.
//     opts_New<<'z'; // numbers all output items starting from zero
// //     opts_New<<'d'; // Detect intersections of PLC facets.
//     opts_New<<'f'; // Outputs all  faces (including non-boundary) 
//     opts_New<<'e'; // Outputs a list of edges of the triangulation
// //     opts_New<<'I'; // Suppresses mesh iteration numbers.
//     opts_New<<'C'; // Checks the consistency of the final mesh.
// //     opts_New<<'Q'; // Quiet: No terminal output except errors.
// //     opts_New<<'g'; // Outputs mesh to .mesh file for viewing by Medit
// //     opts_New<<'Y'; // Suppresses boundary facets/segments splitting
//     opts_New<<'m'; //  
//    
//     
//     opts_New<<'V';  //verbose mode
//     opts_New<<ends;
//     
//     N_G = Out.numberofpoints;   
//     Out.pointmtrlist = new double[N_G];  
//     Out.numberofpointmtrs = 1;
//     
//      for (i=0;i<N_G;i++)
//         Out.pointmtrlist[i] = 10.;
//      
//      Out.pointmtrlist[Out.tetrahedronlist[0]] = 0.025;
//      Out.pointmtrlist[Out.tetrahedronlist[1]] = 0.025;
//      
//     tetrahedralize((char*)opts_New.str().c_str(), &Out, &Out_New);  
//     
//     for (i=0;i<N_G;i++)
//      cout << "Out_New.pointmtrlist " << Out_New.pointmtrlist[i]<< endl;
//     
   } // if(rank==0)
