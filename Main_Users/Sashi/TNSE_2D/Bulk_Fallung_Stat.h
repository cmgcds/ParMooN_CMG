		    ...
		    double **parameters, double **coeffs)
{
  ...
  double k_r = 1e-2;
  double C_nuc = 15.33;
  double d_p_0 = TDatabase::ParamDB->D_P_0;
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    if (param[4] < exp(C_2/d_p_0))
      ...
      B_C_c = k_nuc*(param[4] - exp(C_2/d_p_0))
	  *(param[4] - exp(C_2/d_p_0))*(param[4] - exp(C_2/d_p_0))
	  *(param[4] - exp(C_2/d_p_0))*(param[4] - exp(C_2/d_p_0));
    ...
    coeff[2] = param[1]; // u2
    coeff[3] = 0;
    coeff[4] = c*param[2]*param[3] - C_nuc*d_p_0*d_p_0*d_p_0*c1*B_C_c;//r_chem - r_nuc
  }
}
