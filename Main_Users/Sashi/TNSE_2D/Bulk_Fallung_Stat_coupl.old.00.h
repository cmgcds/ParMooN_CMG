...
void ExampleFile()
{
  ...
  OutPut(" lower " << (int)TDatabase::ParamDB->BULK_D_P_MAX << endl);
		    ...
		    double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    if (param[4] < exp(C_2/d_p_0))
      ...
      B_C_c = k_nuc*(param[4] - exp(C_2/d_p_0))*(param[4] - exp(C_2/d_p_0))*
	  (param[4] - exp(C_2/d_p_0))*(param[4] - exp(C_2/d_p_0))*(param[4] - exp(C_2/d_p_0));
