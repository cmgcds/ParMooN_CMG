...
// flow in a channel across a step
// 
// u(x,y) = ???
// p(x,y) = ???

#define __CHANNELSTEPSLIP__
...
void ExampleFile()
{
  OutPut("Example: ChannelStepSlip.h"<<endl) 
}
// ========================================================================
// initial solution
// ========================================================================
...
void BoundCondition(int i, double t, BoundCond &cond)
{
  ...
  {
  case 5:
     // this is the outflow boundary on the right hand side
     // use outflow bounary conditions
     cond = NEUMANN;
     TDatabase::ParamDB->INTERNAL_PROJECT_PRESSURE = 0;
     break;
  case 6:
     // this is the upper side of the channel
     // it is set to free slip
     // see Assemble2D.C, around line 1260
    cond = SLIP_FRICTION_PENETRATION_RESISTANCE;
    TDatabase::ParamDB->INTERNAL_SLIP_WITH_FRICTION = 1;
    break;
  case 7:
     // this is the inflow boundary on the left hand side
     // the inflow is prescribed
    cond = DIRICHLET;
    break;
  default :
     // these are the lower side of the channel and the step
     // there is slip with friction
    cond = SLIP_FRICTION_PENETRATION_RESISTANCE;
    TDatabase::ParamDB->INTERNAL_SLIP_WITH_FRICTION = 2;
    // the next line has to be outcommented if no slip should be applied
//    cond = DIRICHLET;
    break;
  }
}
                          ...
                          double &reattachment_point)
{
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    for (j=0;j<N_Edges;j++)
    {
      ...
      // for slip b.c.
      if  (TDatabase::ParamDB->INTERNAL_SLIP_WITH_FRICTION < 2)
      {
        if (fabs(y)>eps)
          continue;
      }
