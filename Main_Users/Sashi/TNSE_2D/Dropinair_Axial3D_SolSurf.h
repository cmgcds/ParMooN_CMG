...
)
{
  ...
  double x0, x1, y0, y1, T_val[3], Gamma, T_Marangoni;
...
//   N_FreeBound_Vert = int (TDatabase::ParamDB->P6);    //Freesurf except end point
  ...
  N_TopBound_Vert = 20;
  N_SlipBound_Vert = 60;     // Initially only three points on solid bound (except end point)
...
// exit(0);
  ...
  if(!reparam)
  {
   ...
   for(k=0;k<N_MovVert[2]-1;k++)
    {
     ...
     if(k==N_MovVert[2]-1)
     {  cout<< " last point not available " << endl; exit(0);}
     else
   ...
   { 
//    cout << " reparam b "  << hmin << " " << 0.5*hlimit << " " << hmax << " " << 1.5*hlimit << endl;
