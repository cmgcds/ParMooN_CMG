...
void ExampleFile()
{
  OutPut("Example: MixingLayerSlipIni01.h, U_INFTY"<< U_INFTY<< endl);
}
...
void InitialU1(double x, double y, double *values)
{
  ...
  values[0] -= 0.001* U_INFTY *exp(-z*z/4)*cos(8*Pi*x)*2*y/(sigma*sigma);
  values[0] -= 0.001* U_INFTY *exp(-z*z/4)*cos(20*Pi*x)*2*y/(sigma*sigma);
}
...
void ComputeVorticiyThickness(TFEFunction2D *Vorticity, double *thickness)
{
  ...
  thickness[0] =  U_INFTY/max; // (2*U_INDTY)/(max/2) (divide max by length of
