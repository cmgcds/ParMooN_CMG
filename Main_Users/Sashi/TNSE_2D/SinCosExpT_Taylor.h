...
#define SINCOSEXP_ENDTIME    100000.0

void ExampleFile()
{
  OutPut("Example: SinCosExpT_Taylor.h : N_OSCILLATIONS " << N_OSCILLATIONS ) 
...
// turbulent viscosity (Smagorinsky term)    
    ...
    {
      nu_T_x = (u1_x*u1_xx+u1_y*u1_xy+u2_x*u2_xx+u2_y*u2_xy);
      nu_T_x *= c*c/nu_T;
      nu_T_y = (u1_x*u1_xy+u1_y*u1_yy+u2_x*u2_xy+u2_y*u2_yy);
      nu_T_y *= c*c/nu_T;
    }

// terms coming from the Taylor polynomial approximation
    taylor_1 =  2 *u1_x*u1_xx + 2 * u1_y*u1_xy;
    taylor_1 += u1_xy * u2_x + u1_x * u2_xy + u1_yy * u2_y + u1_y * u2_yy;    
    taylor_2 =  u1_xx * u2_x + u1_x * u2_xx +  u1_xy * u2_y+  u1_y * u2_xy;
    taylor_2 += 2 *u2_x*u2_xy + 2*u2_y*u2_yy;

    coeff[1] = u1_t - (nu + nu_T) *div_1 -(u1_x*nu_T_x + u1_y*nu_T_y)
      + u1 * u1_x + u2 * u1_y + p_x + delta * delta /(2*gamma) * taylor_1;
    coeff[2] = u2_t -  (nu + nu_T)*div_2 -(u2_x*nu_T_x + u2_y*nu_T_y)
      + u1 * u2_x + u2 * u2_y + p_y + delta * delta /(2*gamma) * taylor_2;
