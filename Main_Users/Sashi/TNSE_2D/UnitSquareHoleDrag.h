...
void ExampleFile()
{
  OutPut("Example: UnitSquareHoleDrag.h" << endl) ;
}
...
void InitialU1(double x, double y, double *values)
{
  ...
  values[0] = 10*xm1*xm1*x1*x1*x1*ym1*ym1*ym1*y1*y1*r*
    ((7*y+1)*r+4*y*ym1*y1);
}
...
void InitialU2(double x, double y, double *values)
{
  ...
  values[0] = -10*xm1*x1*x1*ym1*ym1*ym1*ym1*y1*y1*y1*r*
    ((5*x-1)*r+4*x*xm1*x1);
 }
               ...
               double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[0] = eps;
    coeff[1] = 1+sin(x*y*y+y); // f1
    coeff[2] = -0.5*cos(x*x+2*y); // f2
  }
  //OutPut("ff"<<endl);
}
