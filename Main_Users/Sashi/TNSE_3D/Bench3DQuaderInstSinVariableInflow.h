...
void ExampleFile()
{
  OutPut("Example: Bench3DQuaderInstSinVariableInflow.h, U_infty = " << TDatabase::ParamDB->P7 
         << " (ParamDB->P7)" << endl);
  if (TDatabase::ParamDB->P7 <= 0)
  {
     OutPut("U_infty is not positive, change ParamDB->P7" << endl);
     exit(4711);
  }
}
             ...
             double &cd, double &cl)
{
  ...
  double *Der, *aux;
  double uinfty = TDatabase::ParamDB->P7;
  TJoint *joint;
...
// calculation
  ...
  cd *= -20/(0.41*uinfty*uinfty);
  cl *= -20/(0.41*uinfty*uinfty);
