...
void ExampleFile()
{
  OutPut("Example: Calote.h, rotation " << TDatabase::ParamDB->P7 << endl);
}
...
double **parameters, double **coeffs)
{
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    if ((z<=75)&&(z>=65)&&(x*x+y*y<=625))
    {
       coeff[1] = x*2*Pi*TDatabase::ParamDB->P7;
       coeff[2] =-y*2*Pi*TDatabase::ParamDB->P7;
    }
  }
}
