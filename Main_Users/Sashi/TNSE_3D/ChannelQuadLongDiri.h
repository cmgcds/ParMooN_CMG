// Benchmark problem in long channel, Quader, stationary

#define  __CHANNEL_OBSTACLE__
...
void U1BoundValue(double x, double y, double z, double &value)
{
  ...
  if ((fabs(x)<1e-6)||(fabs(x-7.4)<1e-6))
  {
    value = 360*y*(0.41-y)*z*(0.41-z)/(0.41*0.41*0.41*0.41);
  }
  else
  {
    value = 0;
             ...
             double &cd, double &cl)
{
  ...
  vfct = new TFEFunction3D(USpace, "v", "v", v, N_);
...
void L2H1InteriorErrors(int N_Points, double *X, double *Y, double *Z, 
                double *AbsDetjk, 
                ...
                double **coeffs, double *LocError)
{
  ...
  for(i=0;i<N_Points;i++)
  {
    ...
    if (X[i]>TDatabase::ParamDB->P7) continue;
