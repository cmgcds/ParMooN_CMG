...
void ExampleFile()
{
  OutPut("Example: Channel_Tobias.h" << endl);
}
// ========================================================================
// exact solution
// ========================================================================
void InitialU1(double x, double y, double z, double *values)
{
   ...
   double u_tau = 0.0395, y_plus;
   double kappa = 0.41, H = 1; // von Karman constant
   double B = 5.2, cap_pi = 0.1, noise;
   ...
   if (y<=H)
   {
      y_plus = y * u_tau/eps;
       if (y_plus<=30)
      {
         values[0] = log(1+0.4*y_plus)/kappa + 7.8 *(1-exp(-y_plus/11)-y_plus*exp(-0.33*y_plus)/11);      
       }
      else
      {
         if (y <= 0.23* H )
         {
            values[0] = log(y_plus)/kappa + B;
         }
         else
         {
            values[0] = 20.133 +(log(y/H) - 2*cap_pi + (1+6*cap_pi)*(y*y)/(H*H)
                                 -(1+4*cap_pi)*y*y*y/(H*H*H));
         }
      }
   }
   else
   {
      y_plus = (2*H-y) * u_tau/eps; 
      if (y_plus<=30)
      {
         values[0] = log(1+0.4*y_plus)/kappa + 7.8 *(1-exp(-y_plus/11)-y_plus*exp(-0.33*y_plus)/11);      
      }
      else
      {
         if ((2*H-y) <= 0.23* H )
         {
            ...
            values[0] = 20.133 +(log((2*H-y)/H) - 2*cap_pi + (1+6*cap_pi)*((2*H-y)*(2*H-y))/(H*H)
                             -(1+4*cap_pi)*(2*H-y)*(2*H-y)*(2*H-y)/(H*H*H));
      }
   }
   // white noise
   noise = rand();
   noise = 1e-4 *(noise - ((double)(RAND_MAX)/2.0))/double(RAND_MAX);
   values[0] += noise;
}
