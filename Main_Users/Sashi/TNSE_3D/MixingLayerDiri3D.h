...
void ExampleFile()
{
  OutPut("Example: MixingLayerDiri3D.h, U_INFTY "<< U_INFTY<< " NOISE is P7 !!!" 
         << endl);
...
void ComputeMomentumThickness3D(TFEFunction3D *Vorticity, double *thickness)
{
  ...
  double y_min = 1e10, y_max =  -1e10, factor, curr_y;
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    for (j=0;j<N_Faces;j++)
    {
       ...
       x = (X[0]+X[1])/2.0;
       z = (Z[0]+Z[1])/2.0;
       Vorticity->FindGradientLocal(cell, i,x,Y[0],z,val0);
       x = (X[0]+X[3])/2.0;
       z = (Z[0]+Z[3])/2.0;
       Vorticity->FindGradientLocal(cell, i,x,Y[0],z,val1);
       x = (X[2]+X[3])/2.0;
       z = (Z[2]+Z[3])/2.0;
       Vorticity->FindGradientLocal(cell, i,x,Y[0],z,val2);
       x = (X[2]+X[1])/2.0;
       z = (Z[2]+Z[1])/2.0;
       Vorticity->FindGradientLocal(cell, i,x,Y[0],z,val3);
       // opposite vertices
       ...
       x = (X[2]+X[0])/2.0;
       z = (Z[2]+Z[0])/2.0;
       Vorticity->FindGradientLocal(cell, i,x,Y[0],z,val4);
  ...
  integral *= h;
  OutPut("integral "<< integral << endl);
  thickness[0] = (y_max-y_min)/4.0  - integral/(4.0*U_INFTY*U_INFTY);
  OutPut("integral "<< integral << " " << thickness[0] << endl);*/
  integral = 0;
  factor = 2;
 
  for (j=0;j<max_lines+1;j++)
  {
    curr_y = y_min + j*h;
    for (i=0;i<max_lines;i++)
    {     
      if (fabs(global_y_coord[i]-curr_y)<1e-5)
      {
        //OutPut("y " << curr_y << " " << factor << endl);
        integral += factor *   aver_vort[i];
        if (fabs(factor-2)<1e-5)
          factor = 4;
        else
          factor = 2;
        break;
