...
void ExampleFile()
{
  OutPut("Example: UnitSquareHoleDrag3d.h" << endl) ;
}
...
{
  values[0] = 10*(x-1)*(x-1)*(x+1)*(y-1)*(y+1)*sqrt(abs(x*x+y*y-1.0/9.0));
}

void InitialU2(double x, double y, double z, double *values)
{
  values[0] =  -7*(x-1)*(x+1)*(y-1)*(y+1)*(x*x+y*y-1.0/9.0);
}
...
void LinCoeffs(int n_points, double *X, double *Y, double *Z,
  ...
  for(i=0;i<n_points;i++)
  {
    ...
    coeff[0] = eps;
    coeff[1] = 1+sin(x*y*y+y*z); // f1
    coeff[2] = -0.5*z*cos(x*x+2*y); // f2
    coeff[3] = 3*x-2*y*y+z*z*z;
  }
}
