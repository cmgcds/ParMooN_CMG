...
#define  __WALL_MOUNTED_CUBE__
#define __BENCH__

void ExampleFile()
{
  OutPut("Example: WallMountedCube.h" << endl) ;
  if (TDatabase::ParamDB->DRIFT_Z != 0.2)
  {
      TDatabase::ParamDB->DRIFT_Z = 0.2;
      OutPut("Setting DRIFT_Z to " << TDatabase::ParamDB->DRIFT_Z << endl);
  }

  if (TDatabase::ParamDB->N_CELL_LAYERS%2)
  {
      TDatabase::ParamDB->N_CELL_LAYERS++;
      OutPut("Increasing N_CELL_LAYERS to "<<  TDatabase::ParamDB->N_CELL_LAYERS
	     << " since an even number is required" << endl);
  }
  TDatabase::ParamDB->INTERNAL_PROBLEM_IDENTITY = 40000;
...
{
  // conditions at the cube, on the bottom and on the top
  ...
  if ((fabs(y)<1e-6)||(fabs(y-0.7)<1e-6))
    {
      cond  =  SLIP_FRICTION_PENETRATION_RESISTANCE;      
      TDatabase::ParamDB->INTERNAL_SLIP_WITH_FRICTION = 1;
      TDatabase::ParamDB->INTERNAL_SLIP_WITH_FRICTION_IDENTITY = 4;
    }
...
{
  //if ((fabs(x)<1e-6)||(fabs(1.5-x)<1e-6))
  //if ((fabs(x)<1e-6)&& (fabs(z)>1e-6) && (fabs(0.2-z)>1e-6) && (fabs(y)>1e-6) && (fabs(0.7-y)>1e-6)) 
  if ((fabs(x)<1e-6))
  {
    value = 1;
  }
	     ...
	     double &cd, double &cl)
{
  ...
  char VString[] = "v";
  double dt = TDatabase::TimeDB->TIMESTEPLENGTH,x,y,z;
  if (dt < 1e-8)
  {
...
//          (joint->GetType() == IsoBoundface)) // boundary edge 
	...
	if ((comp>=7)&&(comp<=10)) 
          {
	    /*cell->GetVertex(0)->GetCoords(x, y, z);
	    OutPut("1face " << j << " " << comp << " " << x << " " << y << " " <<  z << endl);
	    cell->GetVertex(1)->GetCoords(x, y, z);
	    OutPut("2face " << j << " " << comp << " " << x << " " << y << " " <<  z << endl);
	    cell->GetVertex(2)->GetCoords(x, y, z);
	    OutPut("3face " << j << " " << comp << " " << x << " " << y << " " <<  z << endl);
	    cell->GetVertex(3)->GetCoords(x, y, z);
	    OutPut("4face " << j << " " << comp << " " << x << " " << y << " " <<  z << endl);*/
            //OutPut(comp << " " );
