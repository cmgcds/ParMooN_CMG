...
void ExampleFile()
{
    OutPut("Example: windchannel.00.h " << endl);
...
{
  string name_data_file = "./Inlet1.2bar.txt";
...
//convert the array into a square matrix 
  ...
  if (( TDatabase::ParamDB->WINDCHANNEL_DIM_X!=dim_x)||
      ( TDatabase::ParamDB->WINDCHANNEL_DIM_Y!=dim_y))
  {
  ...
  //x-coordinate of the sorted array 
   ...
   //y-coordinate of the sorted array
   int cordy=(int) (((A[n][2]-min_y)/ny) + 1e-6);
    //write the datas in the array
    TDatabase::ParamDB->WINDCHANNEL_BOUND_VAL[cordx][cordy] = A[n][7]; 
  }
 for(i=0;i<dim_x;i++)
