...
void ExampleFile()
{
    OutPut("Example: windchannel.00_round.h " << endl);
...
{    
    ...
    //for a correct access to the Bounday array they have to be integer in 0- 45 or 0-18 

    if(fabs(x) < eps)
    {
	coord_y= (int) ((y*100+0.5)); // rounding and truncation 
	coord_z= (int) ((z*100+0.5));
	value = TDatabase::ParamDB->WINDCHANNEL_BOUND_VAL[coord_y][coord_z];
	//OutPut(x << " : " << y <<" : "<< coord_y <<" : "  << z << " : " << coord_z  << " :: " << value << endl);
   }
   
    // MAN BEKOMMT HIER (x,y,z)
    // FALLS fabs(x) < eps MUSS ANHAND DER (y,z) KOORDINATEN 
    // BESTIMMT WERDEN, WELCHEN WERT MAN AUS DEM ARRAY WINDCHANNEL_BOUND_VAL
    // NIMMT
