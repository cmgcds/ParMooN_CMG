...
void ExampleFile()
{
  OutPut("Example: windtunnel_00.h " << endl);
  ...
  //TDatabase::ParamDB->GRID_TYPE = 0;
  // OutPut("Setting GRID_TYPE " << TDatabase::ParamDB->GRID_TYPE  <<
  //"GRID_TYPE = 0 equidistant grid" << endl);
...
void U1BoundValue(double x, double y, double z, double &value)
{
  double eps = 1e-6;
  int coord_y, coord_z;
  double dist_y, dist_z;
  double variance, mean_value, stand_dev;
  ...
  if(fabs(x)< eps)
  {
    ...
    coord_y = (int) (y*100 + 0.0001);             //left neighbour
    coord_z = (int) (z*100 + 0.0001);

    dist_y = fabs((double) coord_y - (y * 100.)); //distance between coordinate and left neighbour
    dist_z = fabs((double) coord_z - (z * 100.));
    ...
    variance = (1-dist_y) * (1-dist_z) * TDatabase::ParamDB->WINDTUNNEL_BOUND_VAL[coord_y][coord_z][1]
    ...
    //normal distrubution in each point with the mean value as expected value
    //and variance
...
void ReadExperimentalBoundaryConditions()
{
  ...
  {                                               //coordinates of the sorted array
    ...
    coord_z = (int) ((A[n][2]+0.01)/10);

    TDatabase::ParamDB->WINDTUNNEL_BOUND_VAL[coord_y][coord_z][0] = A[n][7];
                                                  //variance
    TDatabase::ParamDB->WINDTUNNEL_BOUND_VAL[coord_y][coord_z][1] = A[n][8]*A[n][8];
  }
  ...
  /* for(i=0;i<dim_y;i++)
     {for( int j=0;j< dim_z;j++)
      {
      cout << TDatabase::ParamDB->WINDTUNNEL_BOUND_VAL[i][j][0] <<" ";
    } cout << endl;
   }
  exit(-1);*/
}
...
void ReadExperimentalBoundaryConditionsDrops()
{
  ...
  // file which contains the time-averaged boundary data
  ...
  for (i=0; i< dim_y; i++)
    for(j=0; j<dim_z; j++)
      for(r=0; r<dim_r; r++)
      {
        TDatabase::ParamDB->WINDTUNNEL_BOUND_VAL_DROPS[i][j][r][0]= A[i][j][r][0];
    //variance instead of standard deviation
        TDatabase::ParamDB->WINDTUNNEL_BOUND_VAL_DROPS[i][j][r][1]= A[i][j][r][1]*A[i][j][r][1];
      }
...
void accumulate_bound_condition_drops_neu(int N_y, int N_z, int N_r, 
					  double *a_layers_coord)
{
  ...
  double interval_bound, interval_bound_old, interval;
  double acc[N_y+1][N_z+1][dim_r][2], acc1[N_y+1][N_z+1][dim_r][2], acc2[N_y+1][N_z+1][N_r+1][2];
  int comp, firstcomp, i, j,r ,delta;
    ...
    for(j=0;j<dim_z;j++)
  {
    ...
    for(i=0;i<N_y+1;i++)
    {
      ...
      firstcomp=comp;
      //computiation of the new partition, linear transform from dim_y intervals to N_y+1 intervals
      while(comp<(double)(i+1)*(double)(dim_y/(N_y+1.)))
      {
        acc[i][j][r][0]+=TDatabase::ParamDB->WINDTUNNEL_BOUND_VAL_DROPS[comp][j][r][0];
        acc[i][j][r][1]+=TDatabase::ParamDB->WINDTUNNEL_BOUND_VAL_DROPS[comp][j][r][1];
        comp++;
  ...
  for(r=0;r<dim_r;r++)
    for(i=0;i<N_y+1;i++)
  {
    interval_bound_old =0.;
    comp=0;
    for(j=0;j<N_z+1;j++)
    {
      acc1[i][j][r][0]=0;
      acc1[i][j][r][1]=0;
      if(j==0) interval= dim_z/(2.*N_z+1);
      while(comp<(double)(j+1)*(double)(dim_z/(N_z+1.)))
      {
        acc1[i][j][r][0]+=acc[i][comp][r][0];
        acc1[i][j][r][1]+=acc[i][comp][r][1];
        comp++;
  ...
  for(i=0;i<N_y+1;i++)
  {
	...
	acc2[i][j][r][1]=0;
	firstcomp=comp;
	// not maximal internal coordinate
	if(r!=N_r)
	{
	  //linear trasformation, so that the values of the a_layers_coordinates fit to the matrix dimension
	  interval_bound= dim_r/(r_max_exp-a_layers_coord[0])*(a_layers_coord[r+1]-a_layers_coord[0]);
	  interval=(interval_bound+interval_bound_old)/2.;
	}
	else
	  // maximal internal coordinate
	  interval = (dim_r + 1)*a_layers_coord[N_r]/r_max_exp;
	//OutPut(r << " " << comp << " "  << interval<<endl);
	while((comp<interval)&&(comp<WINDTUNNEL_DIM_R_CONST))
	{
	  acc2[i][j][r][0]+=acc1[i][j][comp][0];
	  acc2[i][j][r][1]+=acc1[i][j][comp][1];
	  comp++;
	...
	interval_bound_old=interval_bound;
      }
    }
  }
