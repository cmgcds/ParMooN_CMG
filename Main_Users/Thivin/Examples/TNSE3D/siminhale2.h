...
{
	double radius = 0.0098; // radius = 10mm
	double x_c = 0.10643;
	double z_c = 0.30664;
	...
	if (CompID == 0)
	{
		...
		{
			value = ((z_c+radius) - z) * (z - (z_c - radius)) * ( (x_c + radius) - x ) * (x - (x_c - radius)) * 4.405 * (1.0 / (radius*radius*radius*radius))  ;
			   ...
			   double **parameters, double **coeffs)
{
	...
	for (i = 0; i < n_points; i++)
	{
		...
		coeff[3] = 1.0 / 0.1019367991845056; // f3
	}
}
