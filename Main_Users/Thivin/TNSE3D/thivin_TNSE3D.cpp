...
int main(int argc, char *argv[])
{
   ...
   Output->AddFEFunction(Pressure_FEvect);
    ...
    cout << " End of VTK Function Declaration " <<endl;
    ...
    #ifdef _CUDA
        mesh_deform->updatevertexCoordinates_freeSurf();
        mesh_deform->updatevertexCoordinates_freeSlip();
    #endif // _CUDA
    ...
    // mesh_deform->C_calculateNormal_HostWrapper();
    // mesh_deform->C_calculateNormal_freeSlip_HostWrapper();
    ...
    mesh_deform->get_velocityValues_freeSurface(MeshVelo_FEvect,Velocity_FEvect);
    mesh_deform->AssembleMeshMatrix(fespace_mesh,MeshVelo_FEvect,FactoriseFlag);
    mesh_deform->SolveMeshMatrix(fespace_mesh,MeshVelo_FEvect);
    ...
    {  
        ...
        for(int l=0;l<N_SubSteps;l++)
        {
            ...
            // SystemMatrix_ALE->Strict_impose_FreeSlip_BoundaryCondition(solution,Velocity_FEvect->GetLength(), N_Active);
            ...
            for(int j=1;j<=Max_It;j++)
            {
                ...
                SystemMatrix_ALE->impose_FreeSlip_BoundaryCondition(SystemMatrix_ALE->getRhs(),Velocity_FEvect->GetLength(), N_Active);
                
                FactoriseFlag = (j == 1) ? 1 : 0; 
                mesh_deform->AssembleMeshMatrix(fespace_mesh,MeshVelo_FEvect,FactoriseFlag);
       ...
       if(m==1 || m % TDatabase::TimeDB->STEPS_PER_IMAGE == 0)  
        ...
        double maxDisp = mesh_deform->getMaximumElevation(MeshVelo_FEvect);
            
        outfile << TDatabase::TimeDB->CURRENTTIME * 26  <<"," << (maxDisp - 3.22)*0.13 <<"," << (maxDisp/3.22) * 0.13 <<endl;


    }

    #ifdef  _CUDA
        mesh_deform->clearCudaVariables();
    #endif  //_CUDA
