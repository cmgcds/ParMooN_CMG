  ...
  for(i=0;i<n_points;i++)
    {
        ...
        // coeff[4] = -225.0*x[i]*x[i]*(tanh(15*x[i]*y[i])*tanh(15*x[i]*y[i]) - 1)*sin(x[i])*tanh(15*x[i]*y[i]) - \
        //                      225.0*y[i]*y[i]*(tanh(15*x[i]*y[i])*tanh(15*x[i]*y[i]) - 1)*sin(x[i])*tanh(15*x[i]*y[i]) \
        //                       + 15*y[i]*(1 - tanh(15*x[i]*y[i])*tanh(15*x[i]*y[i]))*sin(x[i]) + \
        //                       15.0*y[i]*(tanh(15*x[i]*y[i])*tanh(15*x[i]*y[i]) - 1)*cos(x[i]) + \
        //                       0.5*sin(x[i])*tanh(15*x[i]*y[i]) + cos(x[i])*tanh(15*x[i]*y[i]);
        coeff[4] = 50 * sin(x[i]) + cos(x[i]) ;
    }
}
