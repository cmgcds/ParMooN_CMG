		...
		//		if(1)
		{
			...
			for (Lev = 1; Lev < MaxLev; Lev++)
			{
				...
				FineFcts[Lev]->GetErrorsOPTPDE(Exact, N_Derivatives,
											   ...
											   kink, upper, lower, LocError);
				delete FineAux;
				if (fabs(LocError[0] - RootError) < 1e-3 * RootError)
					break;
				else
					RootError = LocError[0];
			} // end for Lev < MaxLev
...
/** 
 * determine the value of function at the given point which lies within the
	cell *cell. It does not work for vector values basis funcitons.
	It does not use the static class implementation of reference transformation.
	It creates a new instance of reference transformation, So that multiple threads can 
	operate simuntaneously 
*/
void TFEFunction2D::FindValueLocal_Parallel(TBaseCell *cell, int cell_no,
											double x, double y, double *values)
{
	int i, j, k;
	double xv, yv, xi, eta, eps = 1e-20;
...
void TFEFunction2D::InterpolateSuper(DoubleFunct2D *Exact)
{
	...
	for (i = 0; i < N_Cells; i++)
	{
		...
		case BFUnitTriangle:
			QuadFormula = TFEDatabase2D::GetQFTriaFromDegree(3 * PolynomialDegree);
			N_Edges = 3;
			break;
		}
	...
	double *const errors)
{
	...
	for (int i = 0; i < N_Cells; i++)
	{
		...
		{
		case TriaAffin:
			((TTriaAffin *)F_K)->GetOrigValues(1, &BaseFunct, N_Points, xi, eta, quad_formula_id, &Needs2ndDer);
			break;
		case QuadAffin:
			((TQuadAffin *)F_K)->GetOrigValues(1, &BaseFunct, N_Points, xi, eta, quad_formula_id, &Needs2ndDer);
			break;
		case QuadBilinear:
			((TQuadBilinear *)F_K)->GetOrigValues(1, &BaseFunct, N_Points, xi, eta, quad_formula_id, &Needs2ndDer);
			break;
		default:
