...
/** Constructor */
TFESpace2D::TFESpace2D(TCollection *coll, char *name, char *description) : TFESpace(coll, name, description)
{
	N_ActiveDegrees = 0;
	N_SlaveDegrees = 0;
	UsedElements = NULL;
...
TFESpace2D::TFESpace2D(TCollection *coll, char *name, char *description,
					   ...
					   TCollection *mortarcoll) : TFESpace(coll, name, description)
{
	N_ActiveDegrees = 0;
	N_SlaveDegrees = 0;
	UsedElements = NULL;
	...
	// Thivin -- To Store all the TRefTrans2D objects
	StoreRefTrans();
}
...
TFESpace2D::TFESpace2D(TCollection *coll, char *name, char *description,
					   ...
					   int ord, TCollection *mortarcoll) : TFESpace(coll, name, description)
{
	N_ActiveDegrees = 0;
	N_SlaveDegrees = 0;
	UsedElements = NULL;
	...
	case ContP_PSpace:
		...
		case -2:
			...
			OutPut("Space for triangles not yet implemented !!!!!!" << endl) break;
...
TFESpace2D::TFESpace2D(TCollection *coll, char *name, char *description,
					   ...
					   FE2D *fes, TCollection *mortarcoll) : TFESpace(coll, name, description)
{
	int i, N_;
...
*/
			...
			for (j = 0; j < N_; j++)
			{
				...
				if (type == MortarJoint || type == MortarBaseJoint)
				{
					...
					for (J_loc = 0; J_loc < N2_; J_loc++)
						if (cell3->GetJoint(J_loc) == CurrJoint)
							break;
					...
					if (clip)
					{
						...
						while (cell3->ExistChildren())
						{
							...
							N2_ = TmpLen1[V_loc];
							for (k = 0; k < N2_; k++)
								if (TmpnEoE[TmpVE[MaxLen1 * V_loc + k]] == J_loc)
									break;
	...
	SlaveOffset = N_Inner + l - SumLocDOF - DirichletUpperBound - m + FIRSTMARK;
	InnerOffset = 0 - DirichletUpperBound - m;

	l = N_Inner;
	m = DirichletUpperBound;
	for (i = 0; i < N_DiffBoundNodeTypes; i++)
	{
		BoundOffset[i] = l - m;
		l += N_BoundaryNodes[i];
...
void TFESpace2D::GetDOFPosition(double *x, double *y)
{
	...
	for (i = 0; i < N_Cells; i++)
	{
		...
		case TriaAffin:
			...
			((TTriaAffin *)rt)->GetOrigFromRef(N_Points, xi, eta, X, Y, absdetjk);
			break;

		case TriaIsoparametric:
			((TTriaIsoparametric *)rt)->SetCell(cell);
			((TTriaIsoparametric *)rt)->GetOrigFromRef(N_Points, xi, eta, X, Y, absdetjk);
			break;

		case QuadAffin:
			((TQuadAffin *)rt)->SetCell(cell);
			((TQuadAffin *)rt)->GetOrigFromRef(N_Points, xi, eta, X, Y, absdetjk);
			break;

		case QuadBilinear:
			((TQuadBilinear *)rt)->SetCell(cell);
			((TQuadBilinear *)rt)->GetOrigFromRef(N_Points, xi, eta, X, Y, absdetjk);
			break;

		case QuadIsoparametric:
			((TQuadIsoparametric *)rt)->SetCell(cell);
			((TQuadIsoparametric *)rt)->GetOrigFromRef(N_Points, xi, eta, X, Y, absdetjk);
			break;
		} // endswitch RefTrans
...
// THIVIN
/** return position of all dofs along with the cell number and whether its a boundary joint or not */
...
void TFESpace2D::GetDOFPosition(int dof, double &x, double &y)
{
	...
	for (i = 0; i < N_Cells; i++)
	{
		...
		if (DOFFound > -1) // i.e. dof was found
		{
			...
			case TriaAffin:
				((TTriaAffin *)rt)->SetCell(cell);
				((TTriaAffin *)rt)->GetOrigFromRef(1, xi + DOFFound, eta + DOFFound, &x, &y, absdetjk);
				break;

			case TriaIsoparametric:
				((TTriaIsoparametric *)rt)->SetCell(cell);
				((TTriaIsoparametric *)rt)->GetOrigFromRef(1, xi + DOFFound, eta + DOFFound, &x, &y, absdetjk);
				break;

			case QuadAffin:
				((TQuadAffin *)rt)->SetCell(cell);
				((TQuadAffin *)rt)->GetOrigFromRef(1, xi + DOFFound, eta + DOFFound, &x, &y, absdetjk);
				break;

			case QuadBilinear:
				((TQuadBilinear *)rt)->SetCell(cell);
				((TQuadBilinear *)rt)->GetOrigFromRef(1, xi + DOFFound, eta + DOFFound, &x, &y, absdetjk);
				break;

			case QuadIsoparametric:
				((TQuadIsoparametric *)rt)->SetCell(cell);
				((TQuadIsoparametric *)rt)->GetOrigFromRef(1, xi + DOFFound, eta + DOFFound, &x, &y, absdetjk);
				break;
			} // endswitch RefTrans
...
bool operator==(const TFESpace2D &lhs_space, const TFESpace2D &rhs_space)
{
	if (&lhs_space == &rhs_space) // compare pointers
		return true;
	if ((lhs_space.N_DegreesOfFreedom == rhs_space.N_DegreesOfFreedom) && (lhs_space.N_UsedElements == rhs_space.N_UsedElements) && (lhs_space.BoundCondition == rhs_space.BoundCondition) && (lhs_space.N_ActiveDegrees == rhs_space.N_ActiveDegrees)
...
void TFESpace2D::StoreRefTrans()
{
	...
	RefTrans2DArray = new RefTrans2D[N_Cells];
	TRefTrans2DArray = new TRefTrans2D*[N_Cells];

	for (int i = 0; i < N_Cells; i++)
	{
		cell = Collection->GetCell(i);
		FE_ID = GetFE2D(i, cell);
		FE_Obj = TFEDatabase2D::GetFE2D(FE_ID);
		RefTrans = FE_Obj->GetRefTransID();
		RefTrans2DArray[i] = RefTrans;
		...
		switch (RefTrans)
		{
			...
			case QuadAffin:
				...
				break;
			case QuadBilinear:
				rt = new TQuadBilinear();
				rt->SetCell(cell);
				
				break;
			case TriaIsoparametric:
				rt = new TTriaIsoparametric();
				rt->SetCell(cell);
				
				break;
			case QuadIsoparametric:
				rt = new TQuadIsoparametric();
				rt->SetCell(cell);
				
				break;

			default:
				cout << " [ERROR] - FEfunction2d.C - No such RefTrans found " <<endl;
				break;
		}

		TRefTrans2DArray[i] = rt;
