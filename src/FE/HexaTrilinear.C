...
//Written by Thivin as Function overload to "GetOrigValues" to  Calculate The NODAL Values alone in the Current cell instead 
// of Calculating the Gradients also
void THexaTrilinear::GetOrigValues(double xi, double eta, double zeta,
                               int N_BaseFunct,
                               double *uref,
                               double *uorig,
...
void THexaTrilinear::GetNormalVectors(int JointNumber,double xi,double eta,double zeta,double &n1, double&n2,double &n3, bool NormalizeFlag)
{
  // Get the xi , eta , zeta co-ordinate for the corresponding Joint
  double xi_1,xi_2;
  GetRefValuesfromJointid(JointNumber,xi,eta,zeta,xi_1,xi_2);

  // Get the tangent Vectors in that plane. 
  double t11,t12,t13,t21,t22,t23;
  GetTangentVectors(JointNumber,xi_1,xi_2,t11,t12,t13,t21,t22,t23);
...
//   double len;
  ...
  double t11 = 0, t12 = 0, t13 = 0 ;
  double t21 = 0 , t22 = 0 , t23 = 0 ;
  switch(j)
  {
    case 0:
      Xi = s; Eta = t; Zeta = -1;
...
// THIVIN
void THexaTrilinear::GetRefvaluesfromLocalNodeNumber(FE3D FEid, int local_Node_num,double& xi, double& eta, double& zeta)
{ 
        TFE3D* FE_Obj_mesh = TFEDatabase3D::GetFE3D(FEid);
				TBaseFunct3D* bf_mesh = FE_Obj_mesh->GetBaseFunct3D();
				int N_BaseFunct_mesh = bf_mesh->GetDimension();
        double *localDOFxi,*localDOFeta,*localDOFzeta;
        int N_Points;
        TNodalFunctional3D *nf = TFEDatabase3D::GetNodalFunctional3DFromFE3D(FEid);
        nf->GetPointsForAll(N_Points, localDOFxi, localDOFeta, localDOFzeta);

        if ( local_Node_num > N_BaseFunct_mesh || local_Node_num < 0)
        {
          cout << " ERROR : The local DOF number exceeds the number of DOF's in a Hexaheadral Cell " <<endl;
          cout << " info  : The local DOF in the cell : " << N_BaseFunct_mesh << "the value provided : " 
                                            << local_Node_num <<endl;
          cout << " INFO  :  Error in Function: GetRefvaluesfromLocalNodeNumber , File : Hexatrilinear.c " <<endl;
          exit(0);  // THIVIN - EXIT Statement
        }

        else
        {
            xi    = localDOFxi[local_Node_num];
            eta   = localDOFeta[local_Node_num];
            zeta  = localDOFzeta[local_Node_num];
        }

}

// THIVIN
void THexaTrilinear::GetRefValuesfromJointid(int Jointno ,double xi, double eta, double zeta , double& xi_1 , double& xi_2)
{
  switch (Jointno)
      {
        case 0:
        ...
        default:
        {
          cout << " Error in Class : Hexatrilinear.C - Function : GetRefValuesfromJointid"<<endl;
          cout << " The Given Joint Id for the HEXAHEADRAL cell "<<Jointno  << " Does not exist "<<endl;
        }
      }
}
