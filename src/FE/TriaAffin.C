...
//  for(i=0;i<N_BaseFunct;i++)
//   {
//     cout << i << " rec_detjk " << rec_detjk <<" uxorig : "<< uxorig[i] << endl; 
//   }
...
//   cout << "detjk " << detjk << " rec_detjk " << rec_detjk <<endl;
}
...
//       t2 = 0.5*(x1-x0);
//       t1 = 0.5*(y1-y0);
      t1 = 0.5*(x1-x0);  // bug t2 changed to t1 by Sashi
      t2 = 0.5*(y1-y0);  // bug t1 changed to t2 by Sashi
    break;
...
//       cout << "xc0 " << xc0 << " xc1 " << xc1 << " xc2 " << xc2 << endl;
//      cout << "yc0 " << yc0 << " yc1 " << yc1 << " yc2 " << yc2 << endl; 
//     cout << "zeta " << zeta[i] << " xi " << xi << " eta " << eta  <<" X " << X[i]<< " Y " << Y[i] <<endl;
//     cout<<endl;
   }
