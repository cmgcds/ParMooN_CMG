							  ...
							  double *size_small_scales)
{
  ...
  delete Derivatives_vms_proj_33[0];*/
	...
	OutPut(TDatabase::TimeDB->CURRENTTIME << " size small scales: " << sqrt(small_scales)
										  << " all scales: " << sqrt(all_scales) << " ratio " << sqrt(small_scales) / sqrt(all_scales) << endl);
}
								  ...
								  double *size_small_scales)
{
	...
	OutPut(TDatabase::TimeDB->CURRENTTIME << " size small scales: " << sqrt(small_scales) << " all scales: " << sqrt(all_scales) << " ratio " << sqrt(small_scales) / sqrt(all_scales) << endl);
}
