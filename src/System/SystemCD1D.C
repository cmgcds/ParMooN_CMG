...
TSystemCD1D::TSystemCD1D(int N_L, double start, double end, BoundCond1D *boundConLminLMax, DoubleFunctND *BdValues, char *ParamFile):
                       TSystem1D(N_L, start, end, boundConLminLMax, BdValues, ParamFile)
{
     ...
     for(m=0; m<N_Joints; m++)
      {
       ...
       ((TLineAffin *)F_K)->SetCell(Neigh);
        ...
        for(k=0;k<N_BaseFunct;k++)
         for(l=0;l<N_BaseFunct;l++)
         {
          val = -0.5*g0*JointValues[l]*JointValuesX[k] - 0.5*c0*Epsilon*JointValues[l]*JointValuesX[k]
