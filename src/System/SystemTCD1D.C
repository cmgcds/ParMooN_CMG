...
TSystemTCD1D::TSystemTCD1D(int N_L, double start, double end, BoundCond1D *boundConLminLMax, DoubleFunctND *BdValues, char *ParamFile):
                       TSystemCD1D(N_L, start, end, boundConLminLMax, BdValues, ParamFile)
{
  M_Intl = new TSquareMatrix1D(SqStructure);

  if(TDatabase::ParamDB->DISCTYPE==SUPG)
   {
    S_Intl = new TSquareMatrix1D(SqStructure);
   }  
...
//  FENodalFunction = new TFEFunction1D(FESpace1D, IString, IString, Nodal_sol,  N_NodalPts);
...
//   int *IndexOfNodalPts = FESpace1D->GetIntlPtIndexOfPts();
...
//   FESpace1D->SetIntlPtIndexOfPts(RootPtIndex);
//   FESpace1D->SetN_RootNodalPts(N_NodalPts);
