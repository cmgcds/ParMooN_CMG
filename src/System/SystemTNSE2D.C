						 ...
						 TAuxParam2D *aux, TAuxParam2D *nseaux_error)
{
	...
	// find discrete form
	Disctype = DO_DISCTYPE; // HARDCODED - THIVIN ( Remove it )
	switch (Disctype)
	{
	case GALERKIN:
	...
	case UPWIND:
		...
		DiscreteFormRhs = DiscreteFormRHSAuxDO;
		break;
...
// Added by : Thivin ( 14-Apr-2021 )
// Function : Similar to init Function, but Has additional Parameter for NSE AUX
void TSystemTNSE2D::Init(CoeffFct2D *lincoeffs, BoundCondFunct2D *BoundCond, BoundValueFunct2D *U1BoundValue, BoundValueFunct2D *U2BoundValue,
						 TAuxParam2D *aux, TAuxParam2D *nseaux_error, TAuxParam2D *nseaux_rhs)
{
	...
	// Aux for the RHS part -- DO Equation 
	NSE_Rhsaux = nseaux_rhs;
	...
	InitializeDiscreteForms(DiscreteFormGalerkin,
							...
							DiscreteFormRHSAuxDO,
							LinCoeffs[0], NSEType);
	Disctype = DO_DISCTYPE; // HARDCODED THIVIN ( Change it )
	// find discrete form
	switch (Disctype)
	{
	case GALERKIN:
...
#endif
} // TSystemTNSE2D::Init with NSE AUX RHS PASSED
